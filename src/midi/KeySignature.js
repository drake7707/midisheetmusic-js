...
export class KeySignature {
    ...
    keymap = new Array(160).fill(Accid.None);
    prevmeasure = -1;
    /**
     ...
     */
    static sharpkeys = null;
    static flatkeys = null;
    static trebleSharpNotes = [
    ...
    constructor(num_sharps, num_flats = 0) {
    ...
    static createAccidentalMaps() {
        if (KeySignature.sharpkeys !== null)
            return;
        ...
        function row(entries) {
            const arr = new Array(12).fill(Accid.None);
            for (const [k, v] of Object.entries(entries))
                arr[Number(k)] = v;
            return arr;
        }
        const A = NoteScale.A, As = NoteScale.Asharp, B = NoteScale.B, C = NoteScale.C, Cs = NoteScale.Csharp, D = NoteScale.D, Ds = NoteScale.Dsharp, E = NoteScale.E, F = NoteScale.F, Fs = NoteScale.Fsharp, G = NoteScale.G, Gs = NoteScale.Gsharp;
    ...
    getNumSharps() { return this.num_sharps; }
    getNumFlats() { return this.num_flats; }
    /** Return the white note that should display the given MIDI note number. */
    GetWhiteNote(notenumber) {
    ...
    GetAccidental(notenumber, measure) {
        ...
        if (result === Accid.Sharp) {
            ...
            if (this.keymap[notenumber - 1] === Accid.None &&
    ...
    Notescale() {
        ...
        if (this.num_flats > 0)
            return flatmajor[Math.min(this.num_flats, flatmajor.length - 1)];
    ...
    static Guess(notenums) {
        ...
        for (let key = 0; key < 6; key++) {
            let count = 0;
            for (let n = 0; n < 12; n++) {
                if (KeySignature.sharpkeys[key][n] !== Accid.None)
                    count += notecount[n];
            }
        ...
        for (let key = 0; key < 7; key++) {
            let count = 0;
            for (let n = 0; n < 12; n++) {
                if (KeySignature.flatkeys[key][n] !== Accid.None)
                    count += notecount[n];
            }
            if (count < smallestAccidCount) {
                smallestAccidCount = count;
