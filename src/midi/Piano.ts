...
export class Piano {
  static readonly KeysPerOctave = 7;
  static readonly MaxOctave = 6;
  ...
  private margin         = 0;
  private blackBorder    = 0;
  private blackKeyOffsets: number[] = [];

  private notes: MidiNote[] = [];
  private useTwoColors = false;
  private shade1 = 0xffd2cddc; // rgb(210,205,220)
  private shade2 = 0xff96c8dc; // rgb(150,200,220)
  private maxShadeDuration = 0;
  private showNoteLetters = NoteNameNone;

  // Color constants
  private readonly gray1 = '#606060';
  private readonly gray2 = '#808080';
  private readonly gray3 = '#a0a0a0';

  constructor() {}

  /** Get the preferred pixel size for this piano given screen dimensions. */
  static getPreferredSize(screenWidth: number, _screenHeight: number): { width: number; height: number } {
  ...
  getWidth():  number { return this.margin * 2 + this.blackBorder * 2 + this.whiteKeyWidth * Piano.KeysPerOctave * Piano.MaxOctave; }
  getHeight(): number { return this.margin * 2 + this.blackBorder * 3 + this.whiteKeyHeight; }

  SetMidiFile(midifile: MidiFile, options: MidiOptions): void {
  ...
  SetShadeColors(c1: number, c2: number): void {
    this.shade1 = c1;
    this.shade2 = c2;
  }
  ...
  Draw(ctx: CanvasRenderingContext2D): void {
    if (this.whiteKeyWidth === 0) return;
  ...
  private drawOctaveOutline(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(0, 0);       ctx.lineTo(0, wkh);
    ctx.moveTo(right, 0);   ctx.lineTo(right, wkh);
    ctx.moveTo(0, wkh);     ctx.lineTo(right, wkh);
    ctx.stroke();

    ctx.strokeStyle = this.gray3;
    ctx.beginPath();
    ctx.moveTo(right - 1, 0);  ctx.lineTo(right - 1, wkh);
    ctx.moveTo(1, 0);          ctx.lineTo(1, wkh);
    ctx.stroke();
    ...
    for (let i = 0; i < 10; i += 2) {
      ...
      ctx.strokeStyle = this.gray1;
      ctx.beginPath();
      ctx.moveTo(x1, 0); ctx.lineTo(x1, bkh);
      ctx.moveTo(x2, 0); ctx.lineTo(x2, bkh);
      ctx.moveTo(x1, bkh); ctx.lineTo(x2, bkh);
      ctx.stroke();
      ctx.strokeStyle = this.gray2;
      ctx.beginPath();
      ctx.moveTo(x1 - 1, 0); ctx.lineTo(x1 - 1, bkh + 1);
      ctx.moveTo(x2 + 1, 0); ctx.lineTo(x2 + 1, bkh + 1);
      ctx.moveTo(x1 - 1, bkh + 1); ctx.lineTo(x2 + 1, bkh + 1);
      ctx.stroke();
      ctx.strokeStyle = this.gray3;
      ctx.beginPath();
      ctx.moveTo(x1 - 2, 0); ctx.lineTo(x1 - 2, bkh + 2);
      ctx.moveTo(x2 + 2, 0); ctx.lineTo(x2 + 2, bkh + 2);
      ctx.moveTo(x1 - 2, bkh + 2); ctx.lineTo(x2 + 2, bkh + 2);
      ctx.stroke();
    }
  ...
  private drawBlackKeys(ctx: CanvasRenderingContext2D): void {
    for (let octave = 0; octave < Piano.MaxOctave; octave++) {
      ctx.save();
      ctx.translate(octave * this.whiteKeyWidth * Piano.KeysPerOctave, 0);
      for (let i = 0; i < 10; i += 2) {
  ...
  private drawBlackBorder(ctx: CanvasRenderingContext2D): void {
    ...
    for (let i = 0; i < Piano.KeysPerOctave * Piano.MaxOctave; i++) {
      ctx.fillRect(i * this.whiteKeyWidth + 1, wkh + 2,
        this.whiteKeyWidth - 2, Math.floor(bb / 2));
    }
    ctx.restore();
  }

  private drawNoteLetters(ctx: CanvasRenderingContext2D): void {
  ...
  private shadeOneNote(ctx: CanvasRenderingContext2D, notenumber: number, color: string): void {
    let octave = Math.floor(notenumber / 12) - 2;
    const notescale = notenumber % 12;
    if (octave < 0 || octave >= Piano.MaxOctave) return;
    ...
    switch (notescale) {
      ...
      case 1: // C#
        ...
        if (isGray) { ctx.fillStyle = this.gray2; ctx.fillRect(x1 + 1, bkh - Math.floor(bkh/8), bkw - 2, Math.floor(bkh/8)); }
        break;
      case 2: // D
        x1 = wkw + 2; x2 = bk[1] + 3; x3 = bk[2] - 2;
      ...
      case 3: // D#
        ...
        break;
      case 4: // E
        x1 = wkw * 2 + 2; x2 = bk[3] + 3; x3 = wkw * 3 - 1;
        ctx.fillRect(x2, 0, x3 - x2, bkh + 3);
        ctx.fillRect(x1, bkh + 3, wkw - 3, bottomH);
        break;
      case 5: // F
        x1 = wkw * 3 + 2; x2 = bk[4] - 2;
        ctx.fillRect(x1, 0, x2 - x1, bkh + 3);
        ctx.fillRect(x1, bkh + 3, wkw - 3, bottomH);
        break;
      case 6: // F#
        ...
        break;
      case 7: // G
        x1 = wkw * 4 + 2; x2 = bk[5] + 3; x3 = bk[6] - 2;
        ctx.fillRect(x2, 0, x3 - x2, bkh + 3);
        ctx.fillRect(x1, bkh + 3, wkw - 3, bottomH);
        break;
      case 8: // G#
        ...
        break;
      case 9: // A
        x1 = wkw * 5 + 2; x2 = bk[7] + 3; x3 = bk[8] - 2;
        ctx.fillRect(x2, 0, x3 - x2, bkh + 3);
        ctx.fillRect(x1, bkh + 3, wkw - 3, bottomH);
        break;
      case 10: // A#
        ...
        break;
      case 11: // B
        x1 = wkw * 6 + 2; x2 = bk[9] + 3; x3 = wkw * Piano.KeysPerOctave - 1;
        ctx.fillRect(x2, 0, x3 - x2, bkh + 3);
        ctx.fillRect(x1, bkh + 3, wkw - 3, bottomH);
        break;
    }
  ...
  private findClosestStartTime(pulseTime: number): number {
    let left = 0;
    let right = this.notes.length - 1;
    while (right - left > 1) {
      const i = Math.floor((right + left) / 2);
      if (this.notes[left].getStartTime() === pulseTime) break;
      else if (this.notes[i].getStartTime() <= pulseTime) left = i;
      else right = i;
    }
    while (left >= 1 && this.notes[left - 1].getStartTime() === this.notes[left].getStartTime()) left--;
    return left;
  }

  private nextStartTimeSameTrack(i: number): number {
    ...
    while (j < this.notes.length) {
      if (this.notes[j].getChannel() !== track) { j++; continue; }
      if (this.notes[j].getStartTime() > start) return this.notes[j].getStartTime();
  ...
  private nextStartTime(i: number): number {
    const start = this.notes[i].getStartTime();
    let end = this.notes[i].getEndTime();
    let j = i;
    while (j < this.notes.length) {
  ...
  private colorToCSS(packed: number): string {
   ...
   */
  ShadeNotes(ctx: CanvasRenderingContext2D, currentPulseTime: number, prevPulseTime: number): void {
    ...
    for (let i = lastShadedIndex; i < this.notes.length; i++) {
      ...
      if (start > prevPulseTime && start > currentPulseTime) break;

      if (
        start <= currentPulseTime && currentPulseTime < nextStart && currentPulseTime < end &&
  ...
  ShadeOneNotePublic(ctx: CanvasRenderingContext2D, notenumber: number, color: string): void {
    ctx.save();
    ctx.translate(this.margin + this.blackBorder, this.margin + this.blackBorder);
  ...
  UnShadeOneNote(ctx: CanvasRenderingContext2D, notenumber: number): void {
    ctx.save();
    ctx.translate(this.margin + this.blackBorder, this.margin + this.blackBorder);
    const num = notenumber % 12;
    if (num === 1 || num === 3 || num === 6 || num === 8 || num === 10) {
