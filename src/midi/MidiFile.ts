...
// ---------------------------------------------------------------------------
// MidiOptions – the options that modify how a MIDI file is played / displayed
// ---------------------------------------------------------------------------
export interface MidiOptions {
  /** Microseconds per quarter note (tempo). */
  tempo: number;
  /** Instrument (program number) per logical track. */
  instruments: number[];
  /** Whether each logical track is muted. */
  mute: boolean[];
  /** Which logical tracks to include in sheet-music view. */
  tracks: boolean[];
  /** Semitones to transpose all notes. */
  transpose: number;
  /** Milliseconds window for combining near-simultaneous note start times. */
  combineInterval: number;
  /** Optional override time signature. */
  time: TimeSignature | null;
  /** Combine all tracks into two staves (piano layout). */
  twoStaffs: boolean;
  /** Pulse amount to shift note start times. */
  shifttime: number;
  /** Pulse time to start playback from (seek). */
  pauseTime: number;
  /** If true, keep the original instruments from the MIDI file. */
  useDefaultInstruments: boolean;
  /** Volume (0-100) per logical track; may be null / shorter than tracks. */
  volume: number[] | null;
...
export class MidiDataReader {
  ...
  private checkRead(amount: number): void {
  ...
  ReadInt(): number {
    this.checkRead(4);
    const x =
      ((this.data[this.offset] & 0xff) * 0x1000000) +
...
class ListInt {
  ...
  set(index: number, x: number): void {
    this.data[index] = x;
  }
...
export const InstrumentAbbreviations: string[] = [
...
export class MidiFile {
  ...
  private allevents: MidiEvent[][];
  private tracks: MidiTrack[];
  private trackmode: number;
  private timesig: TimeSignature;
  private quarternote: number;
  private totalpulses: number;
  private trackPerChannel: boolean;
  ...
  getTracks(): MidiTrack[] { return this.tracks; }
  getTime(): TimeSignature { return this.timesig; }
  getFileName(): string { return this.filename; }
  getTotalPulses(): number { return this.totalpulses; }
  ...
  private parse(buffer: ArrayBuffer): void {
    ...
    if (this.tracks.length === 1 && MidiFile.HasMultipleChannels(this.tracks[0])) {
      this.tracks = MidiFile.SplitChannels(
        this.tracks[0],
        this.allevents[this.tracks[0].trackNumber()],
      );
    ...
    if (numer === 0) { numer = 4; denom = 4; }
    this.timesig = new TimeSignature(numer, denom, this.quarternote, tempo);
  }

  private readTrack(file: MidiDataReader): MidiEvent[] {
      ...
      if (eventflag >= EventNoteOn && eventflag < EventNoteOn + 16) {
        ...
        if (mevent.Metaevent === MetaEventTimeSignature) {
          if (mevent.Metalength < 2) {
            throw new Error(
              `MidiFile: Meta Event Time Signature len == ${mevent.Metalength} != 4`,
            );
          }
          ...
          if (mevent.Metalength !== 3) {
            throw new Error(
              `MidiFile: Meta Event Tempo len == ${mevent.Metalength} != 3`,
            );
          }
          mevent.Tempo =
        ...
        throw new Error(
          `MidiFile: Unknown event 0x${eventflag.toString(16)} at offset ${file.GetOffset() - 1}`,
        );
      }
    }
  ...
  static HasMultipleChannels(track: MidiTrack): boolean {
    const notes = track.getNotes();
    if (notes.length === 0) return false;
    const channel = notes[0].getChannel();
    for (const note of notes) {
      if (note.getChannel() !== channel) return true;
    }
    return false;
  }

  /** Write a variable-length integer into buf at offset; return bytes written. */
  static VarlenToBytes(num: number, buf: Uint8Array, offset: number): number {
  ...
  private static intToBytes(value: number, data: Uint8Array, offset: number): void {
  ...
  private static getTrackLength(events: MidiEvent[]): number {
    let len = 0;
  ...
  private static writeEvents(
    ...
    // MThd header
    out[pos++] = 0x4d; out[pos++] = 0x54; out[pos++] = 0x68; out[pos++] = 0x64; // 'MThd'
    MidiFile.intToBytes(6, out, pos); pos += 4;
    ...
    for (const list of allevents) {
      // MTrk header
      out[pos++] = 0x4d; out[pos++] = 0x54; out[pos++] = 0x72; out[pos++] = 0x6b; // 'MTrk'
      const trackLen = MidiFile.getTrackLength(list);
      MidiFile.intToBytes(trackLen, out, pos); pos += 4;

      for (const mevent of list) {
        const varlen = MidiFile.VarlenToBytes(mevent.DeltaTime, buf, 0);
        for (let i = 0; i < varlen; i++) out[pos++] = buf[i];
        ...
        if (mevent.EventFlag === EventNoteOn || mevent.EventFlag === EventNoteOff) {
          ...
          for (let i = 0; i < vl; i++) out[pos++] = buf[i];
          for (let i = 0; i < mevent.Value.length; i++) out[pos++] = mevent.Value[i];
  ...
  private static createTempoEvent(tempo: number): MidiEvent {
    const mevent = new MidiEvent();
    mevent.DeltaTime = 0;
    mevent.StartTime = 0;
    mevent.HasEventflag = true;
  ...
  private static updateControlChange(newevents: MidiEvent[], changeEvent: MidiEvent): void {
    for (const mevent of newevents) {
      if (
        mevent.EventFlag === changeEvent.EventFlag &&
  ...
  private static updateProgramChange(newevents: MidiEvent[], changeEvent: MidiEvent): void {
    for (const mevent of newevents) {
  ...
  private static startAtPauseTime(list: MidiEvent[][], pauseTime: number): MidiEvent[][] {
    const newlist: MidiEvent[][] = [];
   ...
   */
  ChangeSound(options: MidiOptions): Uint8Array {
  ...
  private applyOptionsToEvents(options: MidiOptions): MidiEvent[][] {
    if (this.trackPerChannel) {
      return this.applyOptionsPerChannel(options);
    }

    const numTracks = this.allevents.length;
    const instruments: number[] = new Array(numTracks).fill(0);
    const keeptracks: boolean[] = new Array(numTracks).fill(true);
    const trackVolume: number[] = new Array(numTracks).fill(100);
    ...
    for (let tracknum = 0; tracknum < numTracks; tracknum++) {
      ...
      const filtered: MidiEvent[] = [];

      let channel = -1;
      let origInstrument = 0;
      let foundPC = false;
      let endOfTrackEvent: MidiEvent | null = null;
      ...
      let prevST = 0;
      for (const e of original) {
        if (e.EventFlag !== EventNoteOn && e.EventFlag !== EventNoteOff) continue;
        if (!keeptracks[tracknum]) continue;
        const copy = e.Clone();
  ...
  private applyOptionsPerChannel(options: MidiOptions): MidiEvent[][] {
    const instruments: number[] = new Array(16).fill(0);
    const keepchannel: boolean[] = new Array(16).fill(true);
    const channelVolume: number[] = new Array(16).fill(100);
  ...
  ChangeMidiNotes(options: MidiOptions): MidiTrack[] {
    const newtracks: MidiTrack[] = [];
    const trackMapping: number[] = [];
    ...
    if (options.trackOctaveShift != null) {
      for (let i = 0; i < newtracks.length && i < trackMapping.length; i++) {
        ...
        if (
          origTrack < options.trackOctaveShift.length &&
  ...
  static ShiftTime(tracks: MidiTrack[], amount: number): void {
  ...
  static Transpose(tracks: MidiTrack[], amount: number): void {
    for (const track of tracks) {
      for (const note of track.getNotes()) {
  ...
  private static findHighLowNotes(
    notes: MidiNote[],
    measurelen: number,
    startindex: number,
    starttime: number,
    ...
    while (i < notes.length && notes[i].getStartTime() < endtime) {
      if (notes[i].getEndTime() < starttime) { i++; continue; }
      if (notes[i].getStartTime() + measurelen < starttime) { i++; continue; }
      if (pair.high < notes[i].getNumber()) pair.high = notes[i].getNumber();
      if (pair.low  > notes[i].getNumber()) pair.low  = notes[i].getNumber();
      i++;
    }
  }

  private static findExactHighLowNotes(
    notes: MidiNote[],
    startindex: number,
    starttime: number,
    pair: PairInt,
    ...
    let i = startindex;
    while (i < notes.length && notes[i].getStartTime() < starttime) i++;
  ...
  static SplitTrack(track: MidiTrack, measurelen: number): MidiTrack[] {
    const notes = track.getNotes();
    const count = notes.length;

    const top    = new MidiTrack(1);
    const bottom = new MidiTrack(2);
    const result: MidiTrack[] = [top, bottom];
    ...
    for (const note of notes) {
      ...
      const pair:      PairInt = { high: number, low: number };
      const pairExact: PairInt = { high: number, low: number };
      ...
      if (highExact - number > 12 || number - lowExact > 12) {
        if (highExact - number <= number - lowExact) top.AddNote(note);
        else bottom.AddNote(note);
      } else if (high - number > 12 || number - low > 12) {
        if (high - number <= number - low) top.AddNote(note);
        else bottom.AddNote(note);
        ...
        if (prevhigh - number <= number - prevlow) top.AddNote(note);
        else bottom.AddNote(note);
      }
  ...
  static CombineToSingleTrack(tracks: MidiTrack[]): MidiTrack {
    const result = new MidiTrack(1);
    if (tracks.length === 0) return result;
    if (tracks.length === 1) {
      for (const note of tracks[0].getNotes()) result.AddNote(note);
      return result;
    }

    const noteindex = new Array(tracks.length).fill(0);
    const notecount = tracks.map(t => t.getNotes().length);
    let prevnote: MidiNote | null = null;

    while (true) {
      let lowestnote: MidiNote | null = null;
      let lowestTrack = -1;
      for (let tracknum = 0; tracknum < tracks.length; tracknum++) {
        ...
        if (lowestnote === null) {
          lowestnote = note; lowestTrack = tracknum;
        } else if (note.getStartTime() < lowestnote.getStartTime()) {
          lowestnote = note; lowestTrack = tracknum;
        } else if (
          note.getStartTime() === lowestnote.getStartTime() &&
  ...
  static CombineToTwoTracks(tracks: MidiTrack[], measurelen: number): MidiTrack[] {
  ...
  private static CheckStartTimes(tracks: MidiTrack[]): void {
    for (const track of tracks) {
      let prevtime = -1;
      for (const note of track.getNotes()) {
  ...
  static RoundStartTimes(tracks: MidiTrack[], millisec: number, time: TimeSignature): void {
  ...
  static RoundDurations(tracks: MidiTrack[], quarternote: number): void {
    for (const track of tracks) {
      let prevNote: MidiNote | null = null;
      for (let i = 0; i < track.getNotes().length - 1; i++) {
        const note1 = track.getNotes()[i];
        if (prevNote === null) prevNote = note1;
        ...
        else if (quarternote / 2 <= maxduration) dur = Math.floor(quarternote / 2);
        else if (quarternote / 3 <= maxduration) dur = Math.floor(quarternote / 3);
        else if (quarternote / 4 <= maxduration) dur = Math.floor(quarternote / 4);

        if (dur < note1.getDuration()) dur = note1.getDuration();

        if (
          prevNote.getStartTime() + prevNote.getDuration() === note1.getStartTime() &&
  ...
  private static SplitChannels(origtrack: MidiTrack, events: MidiEvent[]): MidiTrack[] {
    const channelInstruments: number[] = new Array(16).fill(0);
    for (const mevent of events) {
  ...
  GuessMeasureLength(): number[] {
    const result: number[] = [];

    const pulsesPerSecond = Math.floor(
      (1000000.0 / this.timesig.getTempo()) * this.timesig.getQuarter(),
    );
    ...
    for (const track of this.tracks) {
      ...
      if (firstnote > t) firstnote = t;
    }

    const interval = Math.floor(
      (this.timesig.getQuarter() * 60000) / this.timesig.getTempo(),
    );
    ...
    for (const track of this.tracks) {
      ...
      for (const note of track.getNotes()) {
        ...
        if (timeFromFirst < minmeasure) continue;
        if (timeFromFirst > maxmeasure) break;
  ...
  EndTime(): number {
    let lastStart = 0;
    for (const track of this.tracks) {
      if (track.getNotes().length === 0) continue;
      const last = track.getNotes()[track.getNotes().length - 1].getStartTime();
      if (last > lastStart) lastStart = last;
    }
    return lastStart;
  }
  ...
  hasLyrics(): boolean {
    for (const track of this.tracks) {
      if (track.getLyrics() != null) return true;
    }
    return false;
  }

  static hasMidiHeader(data: Uint8Array): boolean {
