...
import { NoteHeight, NoteWidth, LineSpace, LineWidth, } from './SheetMusicConstants';
...
export class ChordSymbol {
    ...
    tiedToPrev = false;
    staccato;
    constructor(midinotes, key, time, c, sheet, startTime) {
            ...
            if (midi.isTiedToPrev()) {
                ...
                for (const nd of this.notedata) {
                    if (nd.number === midi.getNumber()) {
                        if (this.tiedFromPrevNotes === null)
                            this.tiedFromPrevNotes = [];
        ...
        if (dur1 !== dur2) {
            ...
            this.stem1 = new Stem(this.notedata[0].whitenote, this.notedata[change - 1].whitenote, dur1, Stem.Down, ChordSymbol.NotesOverlap(this.notedata, 0, change));
            this.stem2 = new Stem(this.notedata[change].whitenote, this.notedata[this.notedata.length - 1].whitenote, dur2, Stem.Up, ChordSymbol.NotesOverlap(this.notedata, change, this.notedata.length));
        }
        else {
            const direction = ChordSymbol.StemDirection(this.notedata[0].whitenote, this.notedata[this.notedata.length - 1].whitenote, c);
    ...
    static computeStaccato(midinotes) {
        let eligible = 0;
        let staccatoCount = 0;
    ...
    static CreateNoteData(midinotes, key, time) {
    ...
    static CreateAccidSymbols(notedata, clef) {
        const result = [];
    ...
    static StemDirection(bottom, top, clef) {
    ...
    static NotesOverlap(notedata, start, end) {
    ...
    getClef() { return this.clef; }
    getHasTwoStems() { return this.hastwostems; }
    hasTie() { return this.tiedNotes !== null && this.tiedNotes.length > 0; }
    getTiedNotes() { return this.tiedNotes; }
    isTiedToPrev() { return this.tiedToPrev; }
    getTiedFromPrevNotes() { return this.tiedFromPrevNotes; }
    getNoteXRight() {
    ...
    Draw(ctx, ytop) {
        ...
        if (this.stem1)
            this.stem1.Draw(ctx, ytop, topstaff);
        if (this.stem2)
            this.stem2.Draw(ctx, ytop, topstaff);
        if (this.staccato)
            this.DrawStaccato(ctx, ytop, topstaff);
        ctx.restore();
        ctx.restore();
    }
    DrawStaccato(ctx, ytop, topstaff) {
    ...
    DrawNoteLetters(ctx, ytop, topstaff) {
        ...
        for (const note of this.notedata) {
            ...
            ctx.fillText(this.NoteName(note.number, note.whitenote), xnote, ynote + Math.floor(NoteHeight / 2));
        }
    }
    static CanCreateBeam(chords, time, startQuarter) {
        ...
        if (numChords === 3) {
            ...
            if (midStem) {
                if (dur === NoteDuration.Sixteenth && midStem.getDuration() === NoteDuration.Eighth && dur2 === NoteDuration.Sixteenth)
                    mixed_16_8_16 = true;
                else if (dur === NoteDuration.Eighth && midStem.getDuration() === NoteDuration.Sixteenth && dur2 === NoteDuration.Sixteenth)
                    mixed_8_16_16 = true;
                else if (dur === NoteDuration.Sixteenth && midStem.getDuration() === NoteDuration.Sixteenth && dur2 === NoteDuration.Eighth)
                    mixed_16_16_8 = true;
            }
        }
        ...
        if (numChords === 6) {
            if (dur !== NoteDuration.Eighth)
                return false;
            const correctTime = (time.getNumerator() === 3 && time.getDenominator() === 4) ||
            ...
            if (time.getNumerator() === 6 && time.getDenominator() === 4) {
                ...
                if ((chords[0].getStartTime() % beat) > Math.floor(time.getQuarter() / 6))
                    return false;
            }
        }
        else if (numChords === 4) {
            ...
            if (dur === NoteDuration.Eighth)
                ...
                beat = Math.floor(time.getQuarter() / 2);
            if ((chords[0].getStartTime() % beat) > Math.floor(time.getQuarter() / 6))
                return false;
        }
        else if (numChords === 3) {
            const valid = dur === NoteDuration.Triplet ||
        ...
        for (const chord of chords) {
            if (Math.floor(chord.getStartTime() / time.getMeasure()) !== measure)
                return false;
            if (!chord.getStem())
                return false;
    ...
    static CreateBeam(chords, spacing) {
        const firstStem = chords[0].getStem();
        const lastStem = chords[chords.length - 1].getStem();
        let newdirection = -1;
        for (const chord of chords) {
    ...
    static BringStemsCloser(chords) {
        const firstStem = chords[0].getStem();
    ...
    getNotedata() { return this.notedata; }
    toString() {
        ...
        for (const note of this.notedata)
            result += `Note whitenote=${note.whitenote} duration=${note.duration} leftside=${note.leftside} `;
        if (this.stem1)
            result += this.stem1.toString() + ' ';
        if (this.stem2)
            result += this.stem2.toString() + ' ';
        return result;
    }
}
