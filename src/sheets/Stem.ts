...
export class Stem {
  ...
  static readonly LeftSide  = 1;
  static readonly RightSide = 2;

  static readonly PARTIAL_BEAM_NONE      = 0;
  static readonly PARTIAL_BEAM_BOTH_ENDS = 1;
  static readonly PARTIAL_BEAM_RIGHT     = 2;
  static readonly PARTIAL_BEAM_LEFT      = 3;

  private duration: NoteDuration;
  private direction: number;
  private top: WhiteNote;
  private bottom: WhiteNote;
  private end: WhiteNote;
  private notesoverlap: boolean;
  private side: number;
  private pair: Stem | null = null;
  private width_to_pair: number = 0;
  private receiver_in_pair: boolean = false;
  private tripletBeam: boolean = false;
  private partialSixteenthBeam: number = Stem.PARTIAL_BEAM_NONE;

  getDirection(): number { return this.direction; }
  setDirection(value: number): void { this.ChangeDirection(value); }

  getDuration(): NoteDuration { return this.duration; }
  getTop(): WhiteNote { return this.top; }
  getBottom(): WhiteNote { return this.bottom; }
  getEnd(): WhiteNote { return this.end; }
  setEnd(value: WhiteNote): void { this.end = value; }

  getReceiver(): boolean { return this.receiver_in_pair; }
  setReceiver(value: boolean): void { this.receiver_in_pair = value; }

  isTriplet(): boolean { return this.tripletBeam; }
  setTriplet(value: boolean): void { this.tripletBeam = value; }

  getPartialSixteenthBeam(): number { return this.partialSixteenthBeam; }
  setPartialSixteenthBeam(value: number): void { this.partialSixteenthBeam = value; }

  hasMixedOuterSixteenths(): boolean { return this.partialSixteenthBeam !== Stem.PARTIAL_BEAM_NONE; }

  constructor(bottom: WhiteNote, top: WhiteNote, duration: NoteDuration, direction: number, overlap: boolean) {
    this.top          = top;
    this.bottom       = bottom;
  ...
  CalculateEnd(): WhiteNote {
    if (this.direction === Stem.Up) {
      let w = this.top.Add(6);
      if (this.duration === NoteDuration.Sixteenth)   w = w.Add(2);
      if (this.duration === NoteDuration.ThirtySecond) w = w.Add(4);
      return w;
    } else {
  ...
  SetPair(pair: Stem, width_to_pair: number): void {
    this.pair          = pair;
  ...
  Draw(ctx: CanvasRenderingContext2D, ytop: number, topstaff: WhiteNote): void {
    if (this.duration === NoteDuration.Whole) return;
  ...
  private DrawVerticalLine(ctx: CanvasRenderingContext2D, ytop: number, topstaff: WhiteNote): void {
    ...
    if (this.direction === Stem.Up) {
      ...
      ctx.beginPath(); ctx.moveTo(xstart, y1); ctx.lineTo(xstart, ystem); ctx.stroke();
  ...
  private DrawCurvyStem(ctx: CanvasRenderingContext2D, ytop: number, topstaff: WhiteNote): void {
    ctx.lineWidth = 2;
    const xstart = this.side === Stem.LeftSide
      ? Math.floor(LineSpace / 4) + 1
      : Math.floor(LineSpace / 4) + NoteWidth;
    ...
    if (this.direction === Stem.Up) {
      ...
      const drawTail = (ys: number) => {
        ctx.beginPath();
        ctx.moveTo(xstart, ys);
        ctx.bezierCurveTo(
          xstart, ys + Math.floor(3 * LineSpace / 2),
          xstart + LineSpace * 2, ys + NoteHeight * 2,
          xstart + Math.floor(LineSpace / 2), ys + NoteHeight * 3
        );
        ctx.stroke();
      };
      ...
      const drawTail = (ys: number) => {
        ...
        ctx.bezierCurveTo(
          xstart, ys - LineSpace,
          xstart + LineSpace * 2, ys - NoteHeight * 2,
          xstart + LineSpace, ys - NoteHeight * 2 - Math.floor(LineSpace / 2)
        );
        ctx.stroke();
      };
  ...
  private DrawHorizBarStem(ctx: CanvasRenderingContext2D, ytop: number, topstaff: WhiteNote): void {
    ...
    if (this.direction === Stem.Up) {
      ...
      ystart += NoteHeight; yend += NoteHeight;
      ...
      if (this.partialSixteenthBeam === Stem.PARTIAL_BEAM_RIGHT && xend > xstart) {
        ...
        ctx.beginPath(); ctx.moveTo(xmid, ymid); ctx.lineTo(xend, yend); ctx.stroke();
      }
      ...
      if ([NoteDuration.Sixteenth, NoteDuration.ThirtySecond].includes(this.duration)) {
        if (this.partialSixteenthBeam === Stem.PARTIAL_BEAM_BOTH_ENDS && xend > xstart) {
          ...
          const slope = (yend - ystart) / (xend - xstart);
          ctx.beginPath(); ctx.moveTo(xstart, ystart); ctx.lineTo(xstart + plen, Math.round(ystart + slope * plen)); ctx.stroke();
          ctx.beginPath(); ctx.moveTo(xend - plen, Math.round(yend - slope * plen)); ctx.lineTo(xend, yend); ctx.stroke();
        } else if (this.partialSixteenthBeam === Stem.PARTIAL_BEAM_LEFT && xend > xstart) {
          const xmid  = xstart + Math.floor((xend - xstart) / 2);
          const slope = (yend - ystart) / (xend - xstart);
          const ymid  = ystart + Math.round(slope * (xmid - xstart));
          ctx.beginPath(); ctx.moveTo(xstart, ystart); ctx.lineTo(xmid, ymid); ctx.stroke();
        } else {
          ctx.beginPath(); ctx.moveTo(xstart, ystart); ctx.lineTo(xend, yend); ctx.stroke();
        }
      }
      ...
      if (this.isTriplet()) {
        const ybeam = Math.min(
          ytop + topstaff.Dist(this.end) * Math.floor(NoteHeight / 2),
          ytop + topstaff.Dist(this.pair.end) * Math.floor(NoteHeight / 2)
        );
      ...
      if (this.isTriplet()) {
        const ybeam = Math.max(
          ytop + topstaff.Dist(this.end) * Math.floor(NoteHeight / 2) + NoteHeight,
          ytop + topstaff.Dist(this.pair.end) * Math.floor(NoteHeight / 2) + NoteHeight
        );
  ...
  private DrawTripletBracket(
    ctx: CanvasRenderingContext2D,
    xstart: number, xend: number, ybeam: number, above: boolean
  ): void {
    ...
    ctx.beginPath(); ctx.moveTo(xstart, ybracket); ctx.lineTo(xcenter - halfTextWidth, ybracket); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(xcenter + halfTextWidth, ybracket); ctx.lineTo(xend, ybracket); ctx.stroke();

    // Vertical hooks
    const hookLen = Math.floor(NoteHeight / 2);
    if (above) {
      ctx.beginPath(); ctx.moveTo(xstart, ybracket); ctx.lineTo(xstart, ybracket + hookLen); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(xend,   ybracket); ctx.lineTo(xend,   ybracket + hookLen); ctx.stroke();
    } else {
      ctx.beginPath(); ctx.moveTo(xstart, ybracket); ctx.lineTo(xstart, ybracket - hookLen); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(xend,   ybracket); ctx.lineTo(xend,   ybracket - hookLen); ctx.stroke();
    }
